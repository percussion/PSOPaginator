...
public class Paginator extends PSJexlUtilBase implements IPSJexlExpression {
    ...
    /**
     * Counts the number of pages in a body field. The page breaks will be one
     * more than the number of pages, so a field with 1 page break is published
     * as 2 pages.
     * 
     ...
     */
    @IPSJexlMethod(description = "Count the number of page breaks", params = { @IPSJexlParam(name = "fieldContent", description = "Page content") })
    @Deprecated
    ...
    /**
     * Creates the location to a paginated page with the given page number and
     * specified context. If the context is not specified, it is assumed to be
     * 1.
     * 
     * @param location
     *            , the location never <code>null</code>.
     * @param pageno
    ...
    public String createNewLocation(String location, Number pageno,
            String context) {
        ...
        if (StringUtils.isBlank(context)) {
            ...
            log.error(emsg);
            throw new IllegalArgumentException(emsg);
        } else if (StringUtils.isNumeric(context)
                && Integer.parseInt(context) < 0) {
            String emsg = "Context should be numeric and greater than zero: "
                    + pageno;
            log.error(emsg);
            throw new IllegalArgumentException(emsg);
        }
        ...
        return PaginatorUtils.createNewLocation(location, pageno.intValue(),
                contextInt);
    }
    ...
    /**
     * Convenience method for getBodyPageN. Used when the page number parameter
     * is available as a <code>String</code>
     * 
     * @param fieldContent
     *            the body field contents
     * @param pageNumber
     *            the page number. Must be numeric and greater than 0.
     * @return the body page. Never <code>null</code>
     */
    @IPSJexlMethod(description = "Get content from specified page number", params = {
            @IPSJexlParam(name = "fieldContent", description = "Page content"),
     ...
     * @param pageNumber
     *            the page number to retrieve. Must not be <code>null</code>.
    ...
    public String getBodyPageN(String fieldContent, Number pageNumber) {
        ...
        try {
            InputSource in = createInputStreamXML(wrapString(fieldContent));
            String pageHTML = HTMLPaginator.getHTMLPage(in, pageNumber
                    .intValue());
    ...
    /**
     * Convenience method for getSlotPageN. Gets the contents of the slot
     * paginated.
     * 
     * @param item
     *            the parent item which contains the slot. Usually
     *            <code>$sys.item</code>
     * @param slot
     *            the slot name to paginate
     * @param params
     *            a parameters used to evaluate the slot.
     * @param pageSize
     *            the maximum number of slot items per page.
     * @param pageNumber
     *            the number of the page.
     * @return the list of items returned in the slot. Never <code>null</code>.
    ...
    public List<IPSAssemblyResult> getSlotPage(IPSAssemblyItem item,
            String slot, Map<String, Object> params, String pageSize,
            String pageNumber) throws Throwable {
    ...
    @IPSJexlMethod(description = "Get slot contents for a specified page number", params = {
            ...
            @IPSJexlParam(name = "pageSize", description = "number of items per page"),
            @IPSJexlParam(name = "pageNumber", description = "Page number") })
    public List<IPSAssemblyResult> getSlotPageN(IPSAssemblyItem item,
            String slot, Map<String, Object> params, Long pageSize,
            Long pageNumber) throws Throwable {
        initServices();
        ...
        if (start > relItems.size()) {
            log
                    .debug("page starts beyond the end of the list, returning empty slot");
    ...
    /**
     * Convenience method for getSlotPageCountN. Used when the page size is
     * available as a String. Gets the page count on a slot.
     * 
     * @param item
     *            the parent item that contains the slot.
     * @param slot
     *            the name of the slot.
     * @param params
     *            the parameters used to assemble the slot.
     * @param pageSize
     *            the number of items per page.
     * @return the list of items in the slot. Never <code>null</code>. May be
     *         <code>empty</code>.
     * @throws Throwable
     */
    ...
    public Number getSlotPageCountN(IPSAssemblyItem item, String slot,
        ...
        PSOSlotTools tools = new PSOSlotTools();
        List<IPSAssemblyItem> relItems = tools.getSlotContents(item, slot,
                params);
    ...
    /**
     * Convenience methods for createLocationListN(). Used when the page count
     * is available when the page count is available as a String. Creates a list
     * of locations. Appends the "_N" to the based location and produces an
     * expanded list.
     * 
     * @param location
     *            the base location.
    ...
    /**
     * Creates a list of locations. Appends the "_N" to the based location and
     * produces an expanded list.
     * 
     * @param location
     *            the base location.
     * @param pagecount
    ...
    public List<String> createLocationListN(String location, Number pagecount) {
        return PaginatorUtils.createLocationList(location,
    ...
    /**
     * Wraps a string within <code>&lt;div&gt;</code> tag. Used to insure that
     * the page content is well formed.
     * 
     * @param in
     *            the raw string to be wrapped.
